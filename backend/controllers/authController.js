...
const register = async (req, res) => {
  try {
    ...
    const user_id = await userModel.create({ full_name, email, password: hashed, role: "customer", phone });
    const customer_id = await customerModel.create({ user_id, address, nic });
    res.status(201).json({ message: "Registered successfully", user_id, customer_id });
  } catch (err) { res.status(500).json({ message: err.message }); }
};
